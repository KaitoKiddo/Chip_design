...
class Env():
        ...
        for i in range(0, top_connect[:, 1].shape[0]):
            ...
            for k in range(0, daughter_die[:, 1].shape[0]):
                if daughter_die[k, 0] == top_connect[i, 0]:
                    daughter_die_set = np.r_[
                        daughter_die_set, [[daughter_die[k, 1], daughter_die[k, 2], daughter_die[k, 3]]]]
    ...
    def calculate_reward(self, action, count):  # 计算HB和对应端口集的reward
        ...
        if self.port_set_list[count].daughter_die_set[0, 0] != '#':  # 如果daughter找不到top对应端口，视为无连接reward为0
            ...
            daughter_die_set_y = 200 - self.port_set_list[count].daughter_die_set[:, 2].astype(
                'float64')  # 翻转，daughter本地左边转化为全局坐标
            ...
            for i in range(0, daughter_die_points.shape[0]):
                reward = reward + math.sqrt(
                    math.pow((self.HB_centre_points[action, 0] - daughter_die_points[i, 0]), 2) + math.pow(
                        (self.HB_centre_points[action, 1] - daughter_die_points[i, 1]), 2))
                reward1 = reward1 + math.sqrt(
                    math.pow((self.HB_centre_points[action, 0] - daughter_die_points[i, 0]), 2) + math.pow(
                        (self.HB_centre_points[action, 1] - daughter_die_points[i, 1]), 2))
        if self.port_set_list[count].mother_die_set[0, 0] != '#':
            ...
            for j in range(0, mother_die_points.shape[0]):
                reward = reward + math.sqrt(
                    math.pow((self.HB_centre_points[action, 0] - mother_die_points[j, 0]), 2) + math.pow(
                        (self.HB_centre_points[action, 1] - mother_die_points[j, 1]), 2))
                reward1 = reward1 + math.sqrt(
                    math.pow((self.HB_centre_points[action, 0] - mother_die_points[j, 0]), 2) + math.pow(
                        (self.HB_centre_points[action, 1] - mother_die_points[j, 1]), 2))
